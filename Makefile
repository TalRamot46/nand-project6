...
# Makefile for the Hack Assembler (Python Implementation)

# Define variables for filenames and the Python interpreter
ASSEMBLER = Assembler
MAIN_PY = Main.py
PYTHON_INTERPRETER = python3  # Or just 'python' depending on your system

# Default target: create the Assembler executable
all: $(ASSEMBLER)

# Rule to create the Assembler executable (a symbolic link to Main.py)
$(ASSEMBLER): $(MAIN_PY)
	@echo "Creating executable: $(ASSEMBLER)"
	chmod +x $(MAIN_PY)
	ln -sf $(MAIN_PY) $@
	@echo "$(ASSEMBLER) created."

# Rule to run the assembler (example usage - not part of the submission)
run: $(ASSEMBLER)
	@echo "Running the assembler (example):"
	$(PYTHON_INTERPRETER) $(ASSEMBLER) Test.asm  # Replace Test.asm with an actual .asm file
